      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3.5 py-2.5 bg-stone-50 dark:bg-stone-800 border border-stone-300 dark:border-stone-700 rounded-lg text-stone-900 dark:text-stone-100 text-base focus:bg-white dark:focus:bg-stone-800 focus:border-stone-900 dark:focus:border-stone-500 focus:outline-none transition-colors placeholder:text-stone-400 dark:placeholder:text-stone-500"
          />
        </div>
            ...
            {priorityOptions.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-center py-2 px-2.5 rounded-xl text-xs border transition-all cursor-pointer ${
                    isSelected
                      ? opt.activeClass
                      : 'border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-600 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-700 hover:border-stone-300 dark:hover:border-stone-600 font-medium'
                  }`}
                >
                  <span>{opt.label}</span>
                </button>
        ...
        <div className="space-y-2 pt-1">
          ...
          {/* 1. Selected Tags (Clearly visible with remove button) */}
          <div className="flex flex-wrap items-center gap-1.5 p-2 bg-stone-50 dark:bg-stone-800/60 border border-stone-200 dark:border-stone-700 rounded-lg min-h-[38px]">
            {selectedTagNames.length > 0 ? (
              selectedTagNames.map((name) => (
                <span
                  key={name}
                  className="inline-flex items-center gap-1 px-2.5 py-1 bg-stone-900 dark:bg-stone-800 text-white rounded-md text-xs font-semibold shadow-2xs group animate-in fade-in zoom-in-95 duration-150"
                >
                  <span>#{name}</span>
                  <button
                    type="button"
                    onClick={() => removeSelectedTag(name)}
                    className="text-stone-400 hover:text-red-300 p-0.5 rounded-full hover:bg-stone-800 dark:hover:bg-stone-700 transition-colors cursor-pointer"
                    title="移除标签"
                  >
                    <X className="w-3 h-3" />
                  </button>
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-1">
              ...
              {unselectedAvailableTags.map((t) => (
                <button
                  ...
                  className="px-2 py-0.5 rounded-md text-xs bg-white dark:bg-stone-800 text-stone-600 dark:text-stone-300 border border-stone-200 dark:border-stone-700 hover:border-stone-400 dark:hover:border-stone-600 hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors flex items-center gap-0.5 cursor-pointer"
                >
          ...
          <div className="flex items-center gap-1.5 pt-1">
            <input
              ...
              className="px-3 py-1.5 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-lg text-xs text-stone-900 dark:text-stone-100 placeholder:text-stone-400 dark:placeholder:text-stone-500 focus:bg-white dark:focus:bg-stone-800 focus:outline-none focus:border-stone-900 dark:focus:border-stone-500 flex-1 transition-colors"
            />
            <button
              type="button"
              onClick={() => handleAddCustomTag()}
              disabled={!newTagInput.trim()}
              className="px-3.5 py-1.5 bg-stone-900 dark:bg-rose-600 hover:bg-stone-800 dark:hover:bg-rose-700 text-white text-xs font-semibold rounded-lg disabled:opacity-40 transition-colors shrink-0 shadow-xs cursor-pointer"
            >
              + 添加
            </button>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-stone-200 dark:border-stone-800">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm text-stone-600 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-100 hover:bg-stone-100 dark:hover:bg-stone-800 rounded-lg transition-colors font-medium cursor-pointer"
          >
            取消
          </button>
          <button
            ...
            className="px-5 py-2 text-sm bg-stone-900 dark:bg-rose-600 hover:bg-stone-800 dark:hover:bg-rose-700 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 shadow-sm cursor-pointer"
          >
