...
interface SelectableItem {
  id: string;
  category: 'action' | 'topic' | 'tag' | 'person' | 'nav';
  ...
  const items: SelectableItem[] = useMemo(() => {
    ...
    if (mode === 'all' || mode === 'action') {
      const matchedNavs = cleanQ
        ...
        : mode === 'action' ? allNavCommands : (cleanQ ? [] : allNavCommands.slice(0, 4));
    ...
    return list;
  }, [rawQ, mode, cleanQ, tags, topics, people, allNavCommands, onSelectTag, onNavigate, onSelectTopic, onSelectPerson, onOpenQuickCreate, onClose, topicSearchQuery.data]);
      ...
      >
        ...
        <div className="flex items-center px-4 py-3.5 border-b border-stone-200 dark:border-stone-800 bg-stone-50/50 dark:bg-stone-900/90">
          ...
          <input
            type="text"
            autoFocus
            placeholder="输入指令、搜索选题、#赛道、@人物、>动作..."
            value={query}
          ...
          />
          {mode !== 'all' && (
            <span className="mr-2 px-2 py-0.5 rounded text-xs font-semibold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800 shrink-0">
              {mode === 'tag' ? '# 赛道模式' : mode === 'person' ? '@ 人物模式' : '> 动作模式'}
            </span>
          )}
          ...
          {items.length === 0 && (
            <div className="py-12 text-center text-stone-400 dark:text-stone-500 space-y-1">
              ...
              <div className="text-xs text-stone-400 dark:text-stone-500">
                可尝试输入 <strong className="text-stone-600 dark:text-stone-300">#</strong> 查赛道、<strong className="text-stone-600 dark:text-stone-300">@</strong> 查人物、<strong className="text-stone-600 dark:text-stone-300">&gt;</strong> 查动作
              </div>
            </div>
          )}
        </div>
