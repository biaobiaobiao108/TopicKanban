...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <div className="mobile-nav-container md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-stone-200 px-3 pt-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))] flex items-center justify-around">
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            {typeof item.badge === 'number' && item.badge > 0 && (
              <span className="absolute top-0 right-1 min-w-4 h-4 px-1 bg-rose-600 text-white rounded-full text-[9px] font-bold flex items-center justify-center">
                {item.badge}
              </span>
            )}
          </button>
  ...
  return (
    <div className="md:hidden fixed inset-0 z-50 flex">
      ...
      <div ref={drawerRef} role="dialog" aria-modal="true" aria-label="移动端导航菜单" className="mobile-drawer-container relative w-4/5 max-w-xs bg-white h-full shadow-2xl flex flex-col justify-between p-5 z-10 animate-in slide-in-from-left duration-200">
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-stone-900 text-white py-2 rounded-xl text-xs font-semibold shadow-sm cursor-pointer"
            >
            ...
            {onOpenQuickDrops && (
              ...
              >
                <div className="flex items-center gap-2">
                  <Smartphone className={`w-3.5 h-3.5 ${quickDropCount > 0 ? 'text-rose-600' : 'text-stone-500'}`} />
                  <span>手机快投灵感箱</span>
                </div>
                {quickDropCount > 0 ? (
                  ...
                  <span className="text-[10px] text-stone-400 font-normal">7天暂存</span>
                )}
              </button>
            )}
            ...
            <button
              ...
              className="w-full flex items-center gap-2 bg-stone-50 text-stone-700 py-2 px-3 rounded-xl text-xs font-medium border border-stone-200 cursor-pointer"
            >
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <Icon className={`w-4 h-4 ${isActive ? 'text-rose-600' : 'text-stone-400'}`} />
                    <span>{item.label}</span>
                  </div>
                  {typeof item.badge === 'number' && item.badge > 0 && (
                    <span className="text-xs bg-stone-200 text-stone-700 px-1.5 py-0.5 rounded-full font-bold">
                      {item.badge}
                    </span>
                  )}
                </button>
        ...
        {/* Footer Logout */}
        <div className="pt-4 border-t border-stone-100 pb-[env(safe-area-inset-bottom)]">
          <button
            onClick={() => {
              onClose();
              onLogout();
            }}
            className="w-full flex items-center gap-2 text-xs text-red-600 hover:bg-red-50 p-2.5 rounded-xl font-medium transition-colors"
          >
