...
export const TopicDetailHeader: React.FC<TopicDetailHeaderProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-stone-900 border-b border-stone-200 dark:border-stone-800 px-4 sm:px-8 py-2.5 sm:py-3 shrink-0 space-y-2 transition-colors">
      ...
      {/* Bottom Row: Metadata & Next Action (Left + Center) and Updated Timestamp (Right) */}
      <div className="flex items-center justify-between gap-3 pt-0.5 text-[11px] sm:text-xs">
        {/* Left & Center: Stream of Metrics + Centered Next Action */}
        <div className="flex items-center gap-2 text-stone-500 dark:text-stone-400 font-medium flex-wrap min-w-0">
          {totalScore > 0 && (
            <span className="font-mono text-emerald-700 dark:text-emerald-300 font-bold bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.2 rounded border border-emerald-200 dark:border-emerald-800 text-[11px] shrink-0">
              评分 {totalScore}/10
            </span>
          )}
          <ReadinessBadge topic={topic} />
          <span className="text-stone-300 dark:text-stone-700">·</span>
          <span className="shrink-0">{topic.verified_facts_count || 0} 条事实</span>
          <span className="text-stone-300 dark:text-stone-700">·</span>
          <span className="shrink-0">{topic.materials_count || 0} 条素材</span>
          <span className="text-stone-300 dark:text-stone-700">·</span>
          <span className="shrink-0">{topic.draft_word_count || 0} 字</span>
          <span className="text-stone-300 dark:text-stone-700">·</span>

          {/* Centered Next Action / Minimalist Yellow Exclamation Alert */}
          {topic.next_action ? (
            <div className="inline-flex items-center gap-1.5 min-w-0">
              {warning && (
                <span
                  className="text-xs font-bold text-amber-700 dark:text-amber-300 bg-amber-100/80 dark:bg-amber-950/70 border border-amber-300 dark:border-amber-800 px-1.5 py-0.2 rounded-md flex items-center gap-0.5 shrink-0"
                  title={warning}
                >
              ...
              <button
                ...
                className={`group inline-flex items-center gap-1.5 px-2.5 py-0.8 rounded-lg font-semibold transition-all cursor-pointer border text-left max-w-xs sm:max-w-md truncate shadow-2xs hover:scale-[1.02] active:scale-[0.98] ${
                  warning
                    ? 'bg-amber-50/90 dark:bg-amber-950/60 text-amber-950 dark:text-amber-100 border-amber-300 dark:border-amber-700 hover:border-amber-400'
                    : 'bg-rose-50/70 dark:bg-rose-950/40 text-stone-900 dark:text-stone-100 border-rose-200 dark:border-rose-900/70 hover:border-rose-300'
                }`}
                title={`下一步核心行动：${topic.next_action} (已持续 ${getNextActionAgeDays(topic)} 天) - 点击更新或完成`}
              >
                <Zap className={`w-3.5 h-3.5 shrink-0 ${warning ? 'text-amber-500 fill-amber-500' : 'text-rose-600 dark:text-rose-400 fill-rose-600/20'}`} />
            ...
            <button
              type="button"
              onClick={() => setIsActionDialogOpen(true)}
              className="inline-flex items-center justify-center p-1 rounded-md bg-amber-100 dark:bg-amber-950/70 text-amber-700 dark:text-amber-300 border border-amber-300/80 dark:border-amber-700 hover:bg-amber-200 dark:hover:bg-amber-900 hover:scale-110 active:scale-95 transition-all cursor-pointer shadow-2xs shrink-0"
              title="⚠️ 缺少下一步行动 (点击设定)"
            >
              <AlertTriangle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 fill-amber-500/20" />
            </button>
          )}
        </div>

        {/* Right: Updated Timestamp */}
        <div className="shrink-0 text-stone-400 dark:text-stone-500 font-mono text-[11px] hidden sm:block">
