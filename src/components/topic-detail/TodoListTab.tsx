...
const InlineTodoComposer: React.FC<InlineTodoComposerProps> = ({ onCreate, disabled = false }) => {
  ...
  const handleKeyDown = async (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (!nextTitle || isSubmitting || disabled) return;
  ...
  return (
    <label className="flex min-h-10 items-center gap-2 rounded-2xl border border-dashed border-rose-300/80 bg-rose-50/40 px-3 dark:border-rose-900/70 dark:bg-rose-950/20">
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-rose-600/10 text-rose-600 dark:bg-rose-500/15 dark:text-rose-400">＋</span>
      <input
        ref={inputRef}
        value={title}
        ...
        className="min-w-0 flex-1 bg-transparent py-1.5 text-sm text-stone-900 outline-none placeholder:text-stone-400 disabled:cursor-not-allowed disabled:opacity-60 dark:text-stone-100"
      />
      <span className="hidden shrink-0 text-[10px] font-semibold text-stone-400 sm:inline">Enter 添加</span>
    </label>
  );
};
...
const InlineTitleEditor: React.FC<InlineTitleEditorProps> = ({ value, onChange, onKeyDown, onBlur, disabled = false }) => (
  <input
    autoFocus
    value={value}
    ...
    disabled={disabled}
    aria-label="编辑待办标题"
    className="min-w-0 w-full rounded-lg border border-rose-300 bg-white px-2 py-1 text-sm font-semibold text-stone-900 outline-none focus:border-rose-500 dark:border-rose-700 dark:bg-stone-900 dark:text-stone-100"
  />
);

const TodoDragPreview: React.FC<{ todo: TopicTodo; width?: number }> = ({ todo, width }) => (
  <div data-testid="todo-drag-preview" style={width ? { width } : undefined} className="flex min-h-10 items-center gap-2 rounded-2xl border border-rose-300 bg-white px-3 py-1.5 shadow-xl ring-1 ring-rose-500/20 dark:border-rose-700 dark:bg-stone-900">
    <GripVertical className="h-4 w-4 shrink-0 text-rose-400" />
    <span className="min-w-0 flex-1 break-words text-sm font-semibold text-stone-900 dark:text-stone-100">{todo.title}</span>
  </div>
);
...
interface SortableTodoRowProps {
  ...
  onSaveEdit: (todo: TopicTodo) => void;
  onCancelEdit: () => void;
...
const SortableTodoRow: React.FC<SortableTodoRowProps> = ({
  ...
  return (
    <div
      ...
      className={`group flex min-h-10 items-center gap-2 rounded-2xl border px-3 py-1.5 transition-shadow dark:bg-stone-900 ${isDragging ? 'border-rose-300 opacity-0 shadow-none dark:border-rose-700' : 'border-stone-200/70 bg-white dark:border-stone-800'}`}
    >
      <button
        type="button"
        aria-label="拖动排序"
        className="flex h-7 w-7 shrink-0 cursor-grab touch-none items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 hover:text-stone-700 active:cursor-grabbing dark:hover:bg-stone-800 dark:hover:text-stone-200"
        {...attributes}
        {...listeners}
      >
      ...
      {!isEditing && (
        <div className="flex shrink-0 items-center gap-0.5 opacity-100 sm:opacity-0 sm:transition-opacity sm:group-hover:opacity-100">
          <button type="button" onClick={() => onSetCurrent(todo)} disabled={disabled} className="flex h-7 w-7 items-center justify-center rounded-lg text-rose-600 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50 dark:text-rose-400 dark:hover:bg-rose-950/30" title="设为当前行动"><Zap className="h-4 w-4" /></button>
          <button type="button" onClick={() => onEdit(todo)} disabled={disabled} className="flex h-7 w-7 items-center justify-center rounded-lg text-stone-500 hover:bg-stone-100 disabled:cursor-not-allowed disabled:opacity-50 dark:text-stone-400 dark:hover:bg-stone-800" title="编辑待办"><Pencil className="h-4 w-4" /></button>
          <button type="button" onClick={() => onComplete(todo)} disabled={disabled} className="flex h-7 w-7 items-center justify-center rounded-lg text-emerald-600 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50 dark:text-emerald-400 dark:hover:bg-emerald-950/30" title="完成待办"><Check className="h-4 w-4" /></button>
          <button type="button" onClick={() => onDelete(todo)} disabled={disabled} className="flex h-7 w-7 items-center justify-center rounded-lg text-stone-400 hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-rose-950/30 dark:hover:text-rose-400" title="删除待办"><Trash2 className="h-4 w-4" /></button>
        </div>
      )}
    </div>
  );
...
export const TodoListTab: React.FC<TodoListTabProps> = ({ topic, todos, actions, isLoading = false }) => {
  ...
  const [activeTodoWidth, setActiveTodoWidth] = useState<number | undefined>(undefined);
  ...
  const saveEditing = async (todo: TopicTodo) => {
    if (editCancelRef.current || editingTodoId !== todo.id || editSavingRef.current === todo.id) return;
    const title = editingTitle.trim();
    if (!title) {
      showToast({ tone: 'error', message: '待办标题不能为空' });
      return;
    }
    editSavingRef.current = todo.id;
    const success = await run(() => actions.updateTodo(todo.id, { title }));
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    setActiveTodoWidth(event.active.rect.current.initial?.width);
  };

  const handleDragCancel = (_event: DragCancelEvent) => {
    setActiveTodoId(null);
    setActiveTodoWidth(undefined);
  };
      ...
      {isLoading ? <div className="rounded-2xl border border-stone-200/70 bg-white p-8 text-center text-sm text-stone-500 dark:border-stone-800 dark:bg-stone-900">正在加载执行清单…</div> : (
        <>
          <section className="space-y-2">
            ...
            {currentTodo ? (
              <div className="group rounded-2xl border border-rose-300/70 bg-rose-50/70 p-3.5 shadow-2xs dark:border-rose-900/60 dark:bg-rose-950/25">
                  ...
                  <div className="min-w-0 flex-1">
                    {editingTodoId === currentTodo.id ? (
                      <InlineTitleEditor
                        ...
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') { event.preventDefault(); void saveEditing(currentTodo); }
                          if (event.key === 'Escape') { event.preventDefault(); cancelEditing(); }
                        }}
                        onBlur={() => void saveEditing(currentTodo)}
                        disabled={isBusy}
                      />
                    ) : <div className="break-words text-sm font-bold leading-5 text-stone-900 dark:text-stone-100">{currentTodo.title}</div>}
                    <div className="mt-0.5 text-[10px] leading-3 text-stone-500 dark:text-stone-400">可编辑，不会重置行动计时</div>
                  </div>
          ...
          <section className="space-y-2">
            ...
            <DndContext collisionDetection={closestCenter} sensors={sensors} onDragStart={handleDragStart} onDragCancel={handleDragCancel} onDragEnd={(event) => void handleDragEnd(event)}>
              {sortableTodos.length > 0 ? <SortableContext items={sortableTodos.map((todo) => todo.id)} strategy={verticalListSortingStrategy}><div className="space-y-2">{sortableTodos.map((todo) => <SortableTodoRow key={todo.id} todo={todo} isEditing={editingTodoId === todo.id} editingTitle={editingTitle} onEditingTitleChange={setEditingTitle} onEdit={startEditing} onSaveEdit={(item) => void saveEditing(item)} onCancelEdit={cancelEditing} onSetCurrent={(item) => void run(() => actions.setCurrentTodo(item.id))} onComplete={(item) => void run(() => actions.completeTodo(item.id))} onDelete={setDeleteTarget} disabled={isBusy} />)}</div></SortableContext> : <div className="rounded-2xl border border-stone-200/70 bg-white p-5 text-center text-xs text-stone-500 dark:border-stone-800 dark:bg-stone-900 dark:text-stone-400">暂无其他未完成待办</div>}
              {typeof document !== 'undefined' && createPortal(<DragOverlay dropAnimation={null}>{activeTodo ? <TodoDragPreview todo={activeTodo} width={activeTodoWidth} /> : null}</DragOverlay>, document.body)}
            </DndContext>
            <InlineTodoComposer onCreate={createTodo} disabled={isBusy} />
          </section>

          {completedTodos.length > 0 && <section className="rounded-2xl border border-stone-200/70 bg-white dark:border-stone-800 dark:bg-stone-900"><button type="button" onClick={() => setIsCompletedExpanded((expanded) => !expanded)} className="flex min-h-12 w-full items-center justify-between px-4 text-left text-xs font-bold text-stone-600 dark:text-stone-300"><span className="flex items-center gap-2">{isCompletedExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}已完成（{completedTodos.length}）</span><span className="text-[11px] font-normal text-stone-400">保留历史记录，可撤销</span></button>{isCompletedExpanded && <div className="space-y-2 border-t border-stone-100 p-3 dark:border-stone-800">{completedTodos.map((todo) => <div key={todo.id} className="group flex min-h-10 items-center gap-3 rounded-xl px-2.5 py-1.5 hover:bg-stone-50 dark:hover:bg-stone-800/60"><CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" /><div className="min-w-0 flex-1"><div className="break-words text-sm leading-5 text-stone-500 line-through dark:text-stone-400">{todo.title}</div>{todo.completed_at && <div className="mt-0.5 text-[10px] leading-4 text-stone-400">完成于 {todo.completed_at.slice(0, 10)}</div>}</div><div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100"><button type="button" onClick={() => void run(() => actions.reopenTodo(todo.id))} disabled={isBusy} className="flex h-7 w-7 items-center justify-center rounded-lg text-stone-500 hover:bg-stone-100 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-stone-800" title="撤销完成"><RotateCcw className="h-4 w-4" /></button><button type="button" onClick={() => setDeleteTarget(todo)} disabled={isBusy} className="flex h-7 w-7 items-center justify-center rounded-lg text-stone-400 hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-rose-950/30 dark:hover:text-rose-400" title="删除已完成待办"><Trash2 className="h-4 w-4" /></button></div></div>)}</div>}</section>}
        </>
      )}
