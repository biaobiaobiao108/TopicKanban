    ...
    <div className="mx-auto w-full max-w-7xl space-y-5 py-5 sm:py-7">
      <div className="rounded-2xl border border-rose-200/80 bg-rose-50/80 p-4 dark:border-rose-900/60 dark:bg-rose-950/30 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-rose-900/75 dark:text-rose-200/80">Bilibili 使用简体内容，YouTube 使用繁体内容。发布包会自动保存到数据库，请仍在离开前确认已保存。</p>
          </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,25rem)]">
        <form className="min-w-0 space-y-5" onSubmit={(event) => event.preventDefault()}>
          <SectionCard title="Bilibili（简体）" description="标题读取文案创作中的文案标题；简介可按 B 站投稿习惯继续完善。">
          ...
          <SectionCard title="YouTube（繁体）" description="默认由简体标题和简介自动转换；YouTube 简介可独立精简，不会被 B 站内容覆盖。">
          ...
          <SectionCard title="公共内容" description="这些内容可以同时用于两个平台，也可以在复制前按平台习惯调整。" action={<button type="button" onClick={resetFromWorkspace} className="inline-flex min-h-9 items-center gap-1 rounded-lg px-2.5 text-xs font-semibold text-stone-500 hover:bg-stone-100 dark:text-stone-400 dark:hover:bg-stone-800"><RefreshCw className="h-3.5 w-3.5" aria-hidden="true" />恢复自动生成</button>}>
          ...
          <SectionCard title="参考资料" description="默认使用当前工作区资料选择；安全链接和来源元数据始终实时更新。" action={<div className="flex items-center gap-1.5"><button type="button" onClick={() => resetField('source_credits')} className="inline-flex min-h-9 items-center gap-1 rounded-lg px-2 text-xs font-semibold text-stone-500 hover:bg-stone-100 dark:text-stone-400 dark:hover:bg-stone-800"><RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />恢复</button><CopyButton label="复制" compact onCopy={() => void copyText('参考资料', fields.source_credits.filter((source) => source.included).map((source) => `${source.title}${source.url ? `\n${source.url}` : ''}`).join('\n\n'))} /></div>}>
