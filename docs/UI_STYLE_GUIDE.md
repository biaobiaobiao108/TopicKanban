...
这是一套面向开发者的可执行 UI 规范。目标是延续 notebook 的浅色纸面、松柏绿和安静排版，同时保留 TopicKanban 的工作台结构、信息层级与业务行为。
...
- **文人笔记基调**：默认以冷雾青画布、白色表面、石墨文字、松柏绿行动色与朱砂正文标题点缀构成整体视觉；暖沙主题保留暖纸氛围。
...
默认北欧冷杉主题使用 notebook 冷雾青与松柏绿令牌。组件优先使用 CSS 语义令牌；状态色使用 `emerald-*`、`amber-*` 和 `red-*`，不要把 Rose 当作全局品牌强调色。

| 语义 | 默认值 | 用途 |
| --- | --- | --- |
| 工作区背景 | `#f6faf9` | 冷雾青页面画布与融合式侧栏 |
| 表面 | `#ffffff` | 卡片、侧栏、弹窗、输入控件 |
| 主文字 | `#1a2424` | 标题、正文、重要数据 |
| 次文字 | `#5f7474` | 辅助说明、元信息、未激活状态 |
| 弱边框 | `rgba(26, 36, 36, 0.08)` | 轻分隔、控件边界和卡片边界 |
| 强调浅色 | `rgba(53, 107, 91, 0.10)` | 松柏绿行动的浅色选中背景 |
| 强调色 | `#356b5b` | 主操作、焦点和当前行动 |
| 强调深色 | `#285344` | 浅色主题中的强调文字和悬停 |
| 正文 H1 点缀 | `#a34133` | 文案正文 H1 与编辑器标题 |
...
Tokyo Night 深色主题使用工作区 `#1a1b26`、表面 `#24283b`、主文字 `#c0caf5`、次文字 `#a9b1d6`、边框 `#414868` 与蓝色强调 `#7aa2f7`。紫色、青色和绿色用于编辑器标题层级；深色表面仍需和背景形成层级。
...
| Tokyo Night `dark` | `#1a1b26` | `#24283b` | `#7aa2f7` | `#a9b1d6` |
| 跟随系统 `system` | 跟随系统 | 跟随系统 | 跟随系统 | 跟随系统 |

可选项为三套固定主题加跟随系统；跟随系统的浅色外观使用北欧冷杉。
