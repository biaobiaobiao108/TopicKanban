...
### 13. 💾 物理存储健康探测、碎片自动收缩与回收站生命周期 (Storage Compaction & Health)
* **存储物理指标透明化**：设置页内嵌实时探测面板，展示主数据库文件物理大小、WAL 预写日志大小、可收缩空闲页数（Freelist 空间）及回收站积压数。
* **物理空间自动收缩 (VACUUM)**：清空回收站、批量永久删除选题或导入全量备份时，服务端自动执行 `PRAGMA wal_checkpoint(TRUNCATE)` 与 `VACUUM`，彻底解决 SQLite 默认删除后磁盘文件不缩小的物理机制限制；亦可在设置页一键主动优化收缩。
* **回收站生命周期策略**：支持自定义回收站保留期限（7天/14天/30天推荐/60天/从不清理），超期废弃选题在进入回收站时自动级联销毁关联草稿与素材，并自动收缩磁盘空间。
...
| **主业务持久库** | SQLite (`bun:sqlite` + WAL) | 选题、素材、时间线、人物、文案、发布包、商单等业务表（支持自动 VACUUM 与物理收缩） |
