...
### 13. 💾 存储健康探测、空闲页自然复用与主动收缩 (Storage Health & Compaction)
* **空闲页复用优先 (Freelist-first)**：遵循 SQLite 最佳实践，日常单条/批量永久删除选题与回收站超期清理均采用纯净的标准 SQL `DELETE`。释放的页面保留在 SQLite 内部空闲页池（Freelist）中，供后续新建选题与文案撰写时直接原地复用，**杜绝日常高频删除自动重写全库带来的写放大与锁阻塞**。
* **存储物理指标透明化**：设置页内嵌实时探测面板，精准展示主数据库文件物理大小、WAL 预写日志大小、可收缩空闲页数（Freelist 空间）及回收站积压数。
* **主动优化收缩 (Manual VACUUM)**：将物理截断控制权还给用户，在设置页提供「整理并压缩存储」操作，按需执行 `PRAGMA wal_checkpoint(TRUNCATE)` 与 `VACUUM` 物理释放磁盘空间。
* **回收站生命周期治理**：支持自定义回收站保留期限（7天/14天/30天推荐/60天/从不清理），超期废弃选题在进入回收站时自动级联销毁，避免历史垃圾无节制堆积。
* **协同编辑锁内存化隔离**：文案在线编辑的 30 秒感知租约锁（`lock:*`）由内存 LeaseMap 高效维护，彻底消灭高频心跳写库产生的页碎片与 WAL 日志膨胀。

### 14. 🧱 单一数据库基线 (Single Schema Baseline)
* 当前没有生产数据，因此数据库不保留历史迁移链；`drizzle/0000_schema.sql` 是唯一真实 schema，内部通过 `PRAGMA user_version = 2` 标识当前基线。
* 启动时只执行“新库初始化 + 当前基线校验”，不会自动 ALTER、补表或猜测旧字段含义。检测到旧版本或不完整数据库时会明确报错，重新创建本地 `data/kanban.db` 即可恢复。
* JSON 备份同样只接受当前完整 `2.0` 结构，避免数据库 schema、备份 schema 和前端类型各自维护兼容分支。
...
| **键值与临时库** | SQLite `_kv_store` 表 + 内存 LeaseMap | 全局偏好、审稿快照、快投箱；高频在线锁采用内存隔离以消除磁盘碎片 |
...
│   │       ├── bootstrap.ts             # 启动数据聚合
│   │       ├── backup.ts                # 备份导入导出
│   │       ├── system.ts                # 系统数据库探测、存储统计与 VACUUM 空间收缩
