...
import { isTopicStatus } from '../../src/types';
import {
  loadBootstrap,
  loadTrashedTopics,
  loadTopic,
...
app.put('/topics/:id/citations/active', async (c) => {
  try {
    ...
    // Citation metadata is preserved to allow safe undo/redo and reference tracking.
    // Explicit removal of a citation is handled via DELETE /citations/:id.
...
app.post('/topics/:id/share', async (c) => {
  try {
    ...
    return c.json({ success: true, token, url: `/share/${token}`, expires_at: expiresAt, snapshot });
  } catch (error) {
    return jsonError(c, error);
  }
});
...
app.post('/inbox/quick-drop', async (c) => {
  try {
    ...
    try {
      ...
      // 容错处理：接收纯文本或 URL 投递 (如 iOS 快捷指令发送纯文本)
